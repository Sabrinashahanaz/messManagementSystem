...
public class Meal {
    ...
    public boolean save() {
        ...
        try {
            ...
            String mealInsertQuery = "INSERT INTO `meal` "
    ...
    public ArrayList<Meal> allMeals(String month , String year) {
        ...
        Connection con = new DBConnection().connection();
        try {
            String selectQuery = "SELECT DISTINCT(date) FROM `meal` WHERE MONTH(date) = 12 AND YEAR(date) = 2021";
    ...
    public static ArrayList<Meal> getSingleMeal(String date) {
        ...
        Connection con = new DBConnection().connection();
        try {
            String selectQuery = "SELECT * FROM `meal` WHERE date=?";
            PreparedStatement ps = con.prepareStatement(selectQuery);
            ps.setString(1, date);
