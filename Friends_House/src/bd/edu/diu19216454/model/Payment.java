    ...
    public void setForYear(String forYear) {
       ...
       public boolean save() {
        ...
        try {
            ...
            String paymentInsertQuerie = "INSERT INTO `payment`(`paymentId`, `mId`, `amount`, `paymentDate`, `forMonth`, `forYear`) VALUES (NULL,?,?,?,?,?)";
        ...
        Connection con = new DBConnection().connection();
        try {
            String selectQuery = "SELECT `paymentId`, payment.mId, member.name, `amount`, `paymentDate`, `forMonth`, `forYear`"
                    + " FROM `payment`, `member` "
                    + "where payment.mId = member.mId";
