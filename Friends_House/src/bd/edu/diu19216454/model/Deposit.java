...
public class Deposit {
    ...
    public static ArrayList<Deposit> allMember() {
    ...
    public boolean save() {
        ...
        try {
            ...
            String depositInsertQuery = "INSERT INTO `deposit`(`depositId`, `mId`, `amount`, `paymentDate`, `forMonth`, `forYear`) VALUES (NULL,?,?,?,?,?)";
    ...
    public ArrayList<Deposit> allDeposit() {

        ArrayList<Deposit> depositList = new ArrayList<>();

        Connection con = new DBConnection().connection();
        try {
            String selectQuery = "SELECT `depositId`, deposit.mId, member.name, `amount`, `paymentDate`, `forMonth`, `forYear`"
                    + " FROM `deposit`, `member` "
                    + "where deposit.mId = member.mId";
